...
import { message, Spin, Card, Row, Col, Tag, Rate, Divider, Typography, Carousel, Image } from 'antd';
...
const ProductDetail: React.FC = () => {
    ...
    return (
        <div style={{ padding: '20px', maxWidth: '1200px', margin: 'auto' }}>
            <Card
                title={<Title level={1} style={{ textAlign: 'center' }}>{product.name}</Title>}
                bordered={false}
            ...
            >
                <Row gutter={16} style={{ marginTop: '20px' }}>
                    ...
                    <Col span={16} style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-start' }}>
                        <Text style={{ fontSize: '18px', lineHeight: '1.6' }}>{product.description || "Không có mô tả"}</Text>
                        ...
                        <div style={{ marginTop: '20px' }}>
                            <h3>Giảm giá:</h3>
                            <Tag color="green" style={{ fontSize: '16px' }}>{product.discount || 0}%</Tag>
                        </div>
                                ...
                                description={
                                    <div>
                                        {variant.sizes?.map((size: any, sizeIndex: number) => (
                                            <Text key={sizeIndex} style={{ display: 'block', marginBottom: '8px' }}>
